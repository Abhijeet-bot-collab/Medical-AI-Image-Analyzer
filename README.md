...
🧱 Project Architecture :- User uploads image ↓ Streamlit UI (app.py) ↓ Prompt + image metadata ↓ Qwen AI Model (via Bytez API) ↓ Structured medical-style text output ↓ Displayed to user
...
🧱 Project Structure
medical_app/ │ ├── app.py # Streamlit UI logic ├── requirements.txt # Python dependencies ├── .env # API key (local only) │ ├── models/ │ ├── init.py # Makes folder a package │ └── bytez_qwen.py # Qwen + Bytez integration │ └── README.md
